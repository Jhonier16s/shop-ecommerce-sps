...
const ShopSidebar: React.FC<ShopSidebarProps> = ({ data }) => {
  ...
  return (
    <div className=" border-2 border-purpleSps rounded-md flex flex-col pt-10 lg:pt-14 px-6">
      <div className="text-center w-full border-b border-heading pb-8">
      ...
      <div className="space-y-6 py-7 ">
        {/* Address */}
          <div className="block">
            ...
            {!isEmpty(formatAddress(data?.address)) ? (
              <Text className="text-gray-600">{formatAddress(data?.address)}</Text>
          ...
          <div className="block">
            ...
            <div className="flex items-center justify-between">
              {data?.settings?.contact ? (
                <>
                  <Text className="text-gray-400">{data?.settings?.contact}</Text>
                  <button className="font-semibold text-sm text-gray-600 transition-all hover:opacity-80 flex-shrink-0">
                    { t("text-call-now") }
                  </button>
                </>
              ): t('text-no-contact')}
        ...
        {data?.settings?.website && (
          <div className="block">
            ...
            <div className="flex items-center justify-between">
              <Text className="text-gray-600">{data?.settings?.website}</Text>
              <a
