...
const BreadcrumbItem: React.FC<Props> = ({ children, ...props }) => {
	return (
		<li
			className="text-sm text-heading px-2.5 transition duration-200 ease-in first:ps-0 last:pe-0 hover:text-yellowSps"
			{...props}
		>
			{children}
		</li>
...
const BreadcrumbSeparator: React.FC<Props> = ({ children, ...props }) => {
	return (
		<li className="text-base text-heading mt-0.5" {...props}>
			{children}
		</li>
	);
};
